...
export class ProfesorService {
    ...
    async create(profesor: ProfesorEntity): Promise<ProfesorEntity> {
        if (profesor.extension.toString().length === 5) {
            ...
            throw new Error("El profesor no cumple con los requisitos para ser creado");
        }
    }

    async asignarEvaluador(profesor: ProfesorEntity, idEvaluacion: number){
        if (profesor.evaluaciones.length < 3) {
            const evaluacion = await this.profesorRepository.findOne({ where: { id: idEvaluacion } });
            if (!profesor) {
            ...
            throw new Error("El profesor no puede ser asignado a más de 3 evaluaciones");
        }
    }
}
